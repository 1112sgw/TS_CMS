...
const showThisMenuEle = (item, access) => {
  if (item.meta && item.meta.access && item.meta.access.length) {
    if (_.some(item.meta.access, access)) return true
    else return false
  } else return true
}
/**
...
export const showTitle = (item, vm) => config.useI18n ? vm.$t(item.name) : ((item.meta && item.meta.title) || item.name)
...
const hasAccess = (access, route) => {
  if (route.meta && route.meta.access) return _.some(access, route.meta.access)
  else return true
}
